...
public class JavaKafkaSimpleConsumerAPI {
    ...
    public void run(long maxReads,
        ...
        try {
            ...
            LOGGER.info("The first read offset is:{}",readOffSet);
            ...
            while (ever || maxReads > 0) {
                ...
                if (response.hasError()) {
                    ...
                    LOGGER.error("Error fetching data from the Broker:{} Reason:{}", leaderBrokerHost, code);
                ...
                for (MessageAndOffset messageAndOffset : response.messageSet(topic, partitionID)) {
                    ...
                    if (currentOffset < readOffSet) {
                        LOGGER.info("Found and old offset: {}, Expection: {}", currentOffset, readOffSet);
                        continue;
                    }
    ...
    public PartitionMetadata findLeader(
        ...
        for (KafkaBrokerInfo broker : brokers) {
            ...
            try {
                ...
                LOGGER.error("Error communicating with Broker [{}] to find Leader for [{}, {}] Reason:{}", broker.brokerHost, topic, partitionID, e);
            } finally {
                if (consumer != null) {
                    try {
                        consumer.close();
